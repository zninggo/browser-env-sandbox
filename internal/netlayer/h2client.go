...
// No User-Agent default is injected here: a hardcoded UA would disagree with
// the session fingerprint (and the TLS ClientHello version). Callers that
// need a UA must provide it — the sandbox always fills it from the fingerprint.
