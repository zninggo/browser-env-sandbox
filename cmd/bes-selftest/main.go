...
func main() {
	...
	checks := []check{
		...
		// ── Blob-URL Worker (ZMTO-17): blob source must reach worker.go non-empty ──
		{"Blob→createObjectURL stores full source (ZMTO-17)", `(function(){ var b = new Blob(["self.onmessage=function(e){self.postMessage(e.data*3);};"]); var u = URL.createObjectURL(b); return String(URL.__besBlobs[u].length > 0); })()`, "true"},
		{"Blob-URL Worker executes real source (ZMTO-17)", `new Promise(function(res, rej){ var b = new Blob(["self.onmessage = function(e){ self.postMessage(e.data + 100); };"]); var u = URL.createObjectURL(b); var w = new Worker(u); w.onmessage = function(e){ res(String(e.data)); }; w.onerror = function(e){ rej(new Error(String(e.message || e))); }; w.postMessage(5); setTimeout(function(){ rej(new Error("blob worker timeout — source likely empty (ZMTO-17 regression)")); }, 3000); })`, "105"},
		{"createObjectURL stores Blob without _besBytes (ZMTO-17)", `(function(){ var fake = { size: 3, type: "text/plain" }; var u = URL.createObjectURL(fake); return String(URL.__besBlobs[u] !== undefined); })()`, "true"},
