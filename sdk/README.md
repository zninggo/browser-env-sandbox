...
线上实例(如经反向代理终止 TLS 的 `https://example.com`)只需传入带 `https://` 前缀的地址。
...
// 连接 HTTPS 线上实例
sandbox := bes.New("https://example.com")
...
// 连接 HTTPS 线上实例
const sandbox = await Sandbox.create({ serverAddr: 'https://example.com' });
...
# 连接 HTTPS 线上实例
sandbox = Sandbox(server_addr="https://example.com", browser="chrome", os="windows")
